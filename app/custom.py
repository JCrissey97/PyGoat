...
def validate_proxy(request) -> bool:
    """
        Validator for the Test Proxy lesson
        request - a flask request object
    """

    response: bool = (request.method == 'GET' and 'X-Request-Intercepted' in request.headers and request.headers['X-Request-Intercepted'] and 'changeMe' in request.args and request.args['changeMe'] == 'Requests are tampered easily')
    if response:
...
def sqlValidator(user_data, request) -> bool:
    """
...
def phoneHomeValidate(request) -> bool:
    """
    ...
    response: bool = request.form['xsscommentresponse'] == phVal
    if response:
        flash(('success', 'lesson completed'))
    return response
...
def csrf_validate_and_comment(username, request) -> bool:
    """
...
def insecure_deserialization_validate(request) -> bool:
    """
    ...
    if request.method == "POST":
        ...
        # obj = pickle.loads(result) # linter cannot find reference to this variable: unknown purpose
        time.sleep(0.5) 
...
def validate_idor(request) -> bool:
    """
